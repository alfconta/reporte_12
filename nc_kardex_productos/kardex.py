...
class kardex_productos_inventario(models.TransientModel):
  ...
  costo_promedio_inicial=fields.Float('Cost Average Ini:',readonly=True)
  costo_total_inicial=fields.Float('Cost Total Ini:',readonly=True)

  cantidad_final=fields.Float('Quantity End :',readonly=True)
  costo_promedio=fields.Float('Cost Average End:',readonly=True)
...
--------------- EMPIEZA LA UNION 
    ...
    costo_unit
		...
		(
			...
			,sm.value as v_entrada,(sm.value * 0)v_salida,(sm.value *0)v_saldo,sm.state,sm.origin,sm.reference,
		...
		from
		   (
				select sm.id,sm.date_expected,sm.product_id,sm.name,sm.company_id,(sm.product_qty * 0) as u_entrada,
                                 sm.product_qty as u_salida,(sm.price_unit*-1) as costo_unit
                                 ,(sm.value *-1*0)as v_entrada,(sm.value*-1 )v_salida,(sm.value*-1 *0)v_saldo,
				...
				,sl.usage,sl.complete_name,(sm.location_id)as ubicacion,sm.create_uid,sm.inventory_id,sm.picking_id 
				  from stock_move sm  inner join stock_location sl on sm.location_id=sl.id where sl.usage='internal'
  ...
  def _buscar_factura(self):
    for fact in self.obj_kardex:
      if fact.origin:
        query_origen = """
        select Min(id) as id from account_invoice where origin = %s 
        """  
